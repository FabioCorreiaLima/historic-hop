...
ActivityController.generateActivity = async (req, res) => {
    try {
        const options = req.body;
        const activity = await ActivityService.generateSingleActivity(options);
        res.json(_a.formatActivity(activity));
    }
    catch (error) {
...
ActivityController.generateBatch = async (req, res) => {
    try {
        ...
        res.json(activities.map((a) => _a.formatActivity(a)));
    }
    catch (error) {
...
ActivityController.getActivities = async (req, res) => {
    try {
        ...
        const activities = await ActivityRepository.getByPeriod(periodId, Number(limit) || 10);
        res.json(activities.map((a) => _a.formatActivity(a)));
    }
    catch (error) {
