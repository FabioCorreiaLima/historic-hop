...
router.post("/", async (req, res) => {
    try {
        ...
        if (!periodId || !activityType) {
            return res
                .status(400)
                .json({ error: "periodId e activityType são obrigatórios" });
        }
...
// Endpoint específico para gerar mapas históricos
router.post("/map", optionalAuth, async (req, res) => {
    try {
        const { periodId, customPrompt } = req.body;
        if (!periodId) {
            return res.status(400).json({ error: "periodId é obrigatório" });
        }
        const basePrompt = customPrompt || `Mapa histórico detalhado do período ${periodId} do Brasil, mostrando fronteiras, cidades importantes, rotas comerciais, conflitos e migrações. Estilo vintage, educativo, preciso historicamente.`;
        const mapUrl = await AIService.generateMap(basePrompt);
        res.json({ mapUrl, periodId, prompt: basePrompt });
    }
    catch (error) {
        console.error("Erro no mapa:", error);
        res.status(500).json({
            error: error instanceof Error ? error.message : "Erro ao gerar mapa",
        });
    }
});
// Endpoint específico para gerar avatares de personagens
router.post("/avatar", optionalAuth, async (req, res) => {
    try {
        const { periodId, characterName, customPrompt } = req.body;
        if (!periodId) {
            return res.status(400).json({ error: "periodId é obrigatório" });
        }
        const basePrompt = customPrompt || CHARACTER_AVATARS[periodId] || `${characterName || "Personagem histórico"} do período ${periodId}. Retrato realista, detalhado, expressão apropriada à época.`;
        const avatarUrl = await AIService.generateAvatar(basePrompt);
        res.json({ avatarUrl, periodId, characterName, prompt: basePrompt });
    }
    catch (error) {
...
// Endpoint para gerar quiz completo com múltiplas perguntas
router.post("/quiz-set", async (req, res) => {
    try {
        const { periodId, count = 5, level = 1, difficulty = "Fácil", includeMedia = true } = req.body;
        if (!periodId) {
            return res.status(400).json({ error: "periodId é obrigatório" });
        }
        const periodPrompt = PERIOD_PROMPTS[periodId] || "Especialista em História do Brasil.";
...
Gere ${count} perguntas de quiz sobre ${periodId}. Cada pergunta deve ser única e historicamente precisa.
Formato JSON array:
...
[{
  ...
  "imagePrompt": "Descrição para imagem",
  "mapPrompt": "Descrição para mapa",
  "audioPrompt": "Texto para narração"
}]`;
        const output = await AIService.generateText(`${systemPrompt}\n\nJSON:`, 2000);
        ...
        // Gerar mídia para cada pergunta se solicitado
        if (includeMedia) {
            for (let i = 0; i < questions.length; i++) {
                ...
                if (q.imagePrompt) {
                    try {
                        q.imageUrl = await AIService.generateImage(q.imagePrompt, 1024, 1024);
                    }
                    catch (e) {
                        console.error(`Erro na imagem da pergunta ${i}:`, e);
                    }
                }
                // Mapa
                if (q.mapPrompt) {
                    try {
                        q.mapUrl = await AIService.generateMap(q.mapPrompt);
                    }
                    catch (e) {
                        console.error(`Erro no mapa da pergunta ${i}:`, e);
                    }
                }
            }
        }
