...
async function ensureDefaultCurriculum() {
    ...
    await query(`INSERT INTO courses (id, title, slug, order_index, is_published)
     VALUES ($1, $2, $3, 0, TRUE)
     ON CONFLICT (id) DO UPDATE SET title = EXCLUDED.title, slug = EXCLUDED.slug`, [DEFAULT_COURSE_ID, "História do Brasil", "historia-brasil"]);
    const periodsRes = await query(`SELECT id FROM historical_periods ORDER BY order_index ASC, id ASC`);
    let ord = 0;
    for (const row of periodsRes.rows) {
        await query(`INSERT INTO course_periods (course_id, period_id, order_index)
       VALUES ($1, $2, $3)
       ON CONFLICT (course_id, period_id) DO UPDATE SET order_index = EXCLUDED.order_index`, [DEFAULT_COURSE_ID, row.id, ord++]);
        const lessonId = `lesson_${row.id}_main`;
        await query(`INSERT INTO lessons (id, period_id, title, order_index, xp_reward)
       VALUES ($1, $2, $3, 0, 15)
       ON CONFLICT (id) DO NOTHING`, [lessonId, row.id, "Trilha principal"]);
    }
}
async function seedHistoricalPeriods() {
    ...
    for (const p of periods) {
        await query(`INSERT INTO historical_periods 
      ...
      characterEmoji = EXCLUDED.characterEmoji, order_index = EXCLUDED.order_index`, [p.id, p.name, p.emoji, p.years, p.color, p.bgColor, p.borderColor, p.description, p.characterName, p.characterEmoji, p.order_index]);
    }
}
